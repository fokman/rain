        ...
        if (data != null && !data.isEmpty()) {
            ...
            for (Iterator iter = set.iterator(); iter.hasNext(); ) {
                ...
                if (key.endsWith("_id") || key.endsWith("_user")) {//字段以_id结尾的转字符串
                    rowMap.put(HumpNameUtils.toCamelCase(key), String.valueOf(data.get(key)));
                } else {//日期类型格式化
                    Object parameter = data.get(key);
                    if (parameter instanceof java.util.Date) {
                        rowMap.put(HumpNameUtils.toCamelCase(key), DateUtils.formatTime(parameter));
                    } else if (parameter instanceof Date) {
                        rowMap.put(HumpNameUtils.toCamelCase(key), DateUtils.formatTime(parameter));
                    } else if (parameter instanceof Timestamp) {
                        rowMap.put(HumpNameUtils.toCamelCase(key), DateUtils.formatTime(parameter));
                    } else {
                        rowMap.put(HumpNameUtils.toCamelCase(key), data.get(key));
                    }
                }
            }
        }
