...
public class IceClientUtils {
    ...
    public static Properties getProperties(String config) {
        ...
        @Cleanup InputStream in = EnvUtils.getEnvResource(config);
        try {
            prop.load(in);
        } catch (IOException e) {
            log.error("{}", e);
    ...
    public static Communicator getICECommunictor() {
        if (ic == null) {
            synchronized (IceClientUtils.class) {
                if (ic == null) {
                    if (iceLocator == null) {
                        Properties icegrid = getProperties("config.grid");
                        if (icegrid != null) {
                            iceLocator = icegrid.getProperty("Ice.Default.Locator");
                        }
                        if (iceLocator == null)
    ...
    public static String doService(IceRequest iceRequest, String servcieName) {
        try {
            ...
            MessageServicePrx proxy = new _MessageServicePrxI();
            Method m1 = proxy.getClass().getDeclaredMethod("checkedCast", ObjectPrx.class, String.class);
    ...
    public static void main(String[] args) {
        Properties icegrid = getProperties("config.grid");
        if (icegrid != null) {
            String iceLocator = icegrid.getProperty("Ice.Default.Locator");
